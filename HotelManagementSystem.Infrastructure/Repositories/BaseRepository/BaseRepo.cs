...
namespace HotelManagementSystem.Infrastructure.Repositories.BaseRepository
{
    public class BaseRepo<T>(AppDbContext db) : IBaseRepo<T> where T : BaseEntity
    {
        ...
        public async Task<int> CreateAsync(T entity, string username)
        {
        ...
        public async Task<IEnumerable<int>> CreateAsync(IEnumerable<T> entities, string username)
        {
            List<int> list = [];
        ...
        public IQueryable<T> FilterAllAsync(Expression<Func<T, bool>>? search, Expression<Func<T, object>>? orderBy, string OrderByDirection = "ASC")
        {
        ...
        public async Task<T> FilterAsync(Expression<Func<T, bool>>? search, Expression<Func<T, object>>? orderBy, string OrderByDirection = "ASC")
        {
            var query = GetTableNoTracking();
            if (orderBy is not null)
            {
